...
const AddtoCart = observer((props: IProps) => {
  ...
  return (
    <div
      id={'addtoCart'}
      className={cn({ [s['addto']]: true, [s['addto__active']]: isVisible })}
    >
      <div className={cn(s['addto__title'])}>
        {list.length == 0 ? (
          <img src={cart_empty} alt='Корзина пуста' className={s.cart__img} />
        ) : (
          <img src={cart_full} alt='Полная корзинка' className={s.cart__img} />
        )}
      </div>
      <div className={cn(s['addto__goods'], s['cart__goods'])}>
        {list.length != 0 ? (
                ...
                return (
                  <div key={el.id} className={s['goods__item']}>
                    <div className={s['goods__image']}>
                      <a
                        href='http://lorempixel.com/160/160/nature'
                        title={el.name}
                      >
                        <img
                          src={el.img[0]}
                          alt={el.name}
                          className={s['goods-image-icon']}
                        />
                      </a>
                    </div>
                    <div className={s['goods__shop']}>
                      <a
                        href='https://abricoss.ru/goods/Arahis-blanshirovannyj-3'
                        className={s['goods__name']}
                        title={el.name}
                      >
                        {el.name}
                      </a>
                      <div className={s['goods__mod']}>{el.quantity}</div>
                      <div className={s['goods__priceBox']}>
                        ...
                        <div className={s['goods__count']}>1 шт.</div>
                        <a
                          href='#'
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            productsStore.cart.delete(el.id);
                          }}
                        >
                          &times;
                        </a>
          ...
          <div className={cn(s['goods__empty'])}>
            ...
            <a href='/' className={cn(s['btn'])} title='Продолжить покупки'>
              Продолжить покупки
            </a>
          </div>
        )}
