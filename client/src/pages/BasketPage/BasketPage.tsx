    ...
    <>
      ...
      {list.length == 0 ? (
        ...
        <div className={cn(s.basket)}>
          <p className={cn(s.basket_title)}>Корзина</p>
      ...
      <div className={cn(s.order_information)}>
        <div className={cn(s.buy_form)}>
          ...
          >
            ...
            <input className={cn(s.item)} placeholder="Имя" type='text' id='name' />
            <input className={cn(s.item)} placeholder="Телефон" type='number' id='phone' />
            <input className={cn(s.item)} placeholder="E-mail" type='text' id='email' />
            <input className={cn(s.item)} placeholder="Адрес доставки" type='text' id='adress' />
            <div className={cn(s.item)}>
              <input placeholder="Дата доставки" type='text' id='data'/>
            </div>
            <span className={cn(s.option_price)}>Оплата наличными при получении</span>
          </form>
          ...
          <div className={cn(s.price_info)}>
            <span className={cn(s.active)}>Стоимость заказа</span>
            <div className={cn(s.text)}>
              <span className={cn(s.text_left)}>Стоимость</span>
              <span className={cn(s.price_right)}>1230 р</span>
            </div>
            <div className={cn(s.text)}>
              <span className={cn(s.text_left)}>Стоимость</span>
              <span className={cn(s.price_right)}>1230 р</span>
            ...
            <div className={cn(s.text, s.active)}>
              <span className={cn(s.text_left)}>Итого к оплате</span>
              <span className={cn(s.price_right)}>1230 р</span>
            </div>
            <textarea name="comment" id="comment" placeholder="Комментарий к заказу"></textarea>
            <button className={cn(s.cart_button)}>Оформить заказ</button>
          </div>
        </div>
        <div className={cn(s.text_notice)}>
          <span>Для подтверждения заказа и уточнения времени доставки с вами свяжется менеджер нашего магазина</span>
        </div>
      </div>
    </>
  );
